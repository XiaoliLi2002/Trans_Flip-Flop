...
def transformer_flip_flop_task(train_type: str, val_type: str, rdseed: int, num_epochs: int, batch_size: int, dim: int, depth: int, heads: int):
    result={'error_rate':np.zeros(num_epochs)}
    ...
    if train_type=='train_mix':
        train_dataset = dataset['train'].shuffle(rdseed).select(range(160000))
    else:
        train_dataset = dataset['train'].shuffle(rdseed).select(range(160000))  # seed= 42, 142, 1142, 11142, 111142

    eval_dataset = dataset[val_type]
    ...
    for epoch in range(num_epochs):
        ...
        error_rate[epoch]=evaluate(model, eval_dataloader, device)
...
if __name__ == "__main__":
    ...
    default_val_type='val_sparse'
    val_type=query.get("val_type",default_val_type)
